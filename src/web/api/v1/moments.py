...
def _to_out(m: Moment, user: User, *, completed_today: Optional[bool] = None) -> MomentOut:
    tz = _safe_zone(user.timezone)
    is_habit = m.rrule is not None
    if completed_today is None:
        completed_today = (not is_habit) and m.status == "done"
    next_reminder = compute_next_reminder(m, user)
    now_utc = datetime.now(tz=ZoneInfo("UTC"))
    is_overdue = (
        not is_habit
        and m.status == "active"
        and m.occurs_at is not None
        and m.occurs_at < now_utc
    )
    return MomentOut(
        id=m.id,
        ...
        next_reminder_at_local=_to_local_iso(next_reminder, tz),
    )
...
async def create_moment(
    ...
    # Fire-and-forget: автоизвлечение фактов о пользователе. Своя сессия,
    # свой роутер — не блокируем HTTP-ответ.
    asyncio.create_task(
...
async def list_moments(
    ...
    completed_map = await service.completed_today_map(user, items)
    next_cursor = items[-1].id if len(items) == limit and view == "timeline" else None
    return MomentsListOut(
        items=[
            _to_out(m, user, completed_today=completed_map.get(m.id, False))
            for m in items
        ],
...
async def complete_moment(
    ...
    try:
        ...
        _raise_forbidden()
    return _to_out(m, user, completed_today=True)


@router.post("/{moment_id}/uncomplete", response_model=MomentOut)
async def uncomplete_moment(
    moment_id: int,
    user: User = Depends(get_current_user),
    service: MomentService = Depends(get_moment_service),
) -> MomentOut:
    try:
        m = await service.uncomplete(user, moment_id)
    except MomentNotFound:
        _raise_not_found()
    except MomentForbidden:
        _raise_forbidden()
    return _to_out(m, user, completed_today=False)
