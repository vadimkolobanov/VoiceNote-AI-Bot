...
# --- НОВАЯ ФУНКЦИЯ: Инкапсулируем логику генерации утренней сводки ---
async def generate_digest_text(user_name: str, weather_forecast: str, notes_for_prompt: str, bdays_for_prompt: str) -> dict:
...
**ПРАВИЛА:**
1.  Будь кратким, позитивным и используй HTML-теги `<b>` для заголовков и `<i>` для акцентов.
2.  Используй `\n` для переноса строки. Не используй теги `<br>`.
3.  Не пиши лишних вступлений вроде "Вот ваша сводка:". Сразу начинай с приветствия.
4.  Начни с погоды, если она доступна и не является ошибкой. Используй иконку 🌦️.
...
{bdays_for_prompt}
---
Сгенерируй финальное HTML-сообщение.
"""
