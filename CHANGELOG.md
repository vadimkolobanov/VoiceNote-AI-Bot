# Журнал изменений

Все заметные изменения в этом проекте будут задокументированы в этом файле.

Формат основан на [Keep a Changelog](https://keepachangelog.com/ru/1.0.0/),
и этот проект придерживается [Семантического Версионирования](https://semver.org/lang/ru/spec/v2.0.0.html).

## [Unreleased]

### Добавлено
- Документация: добавлены файлы `docs/ARCHITECTURE.md`, `docs/DATABASE.md`, `docs/ROADMAP.md`, `docs/KNOWN_ISSUES.md`.

### Изменено
- Обновлён `README.md`: вынесены планы развития в `docs/ROADMAP.md`, добавлены ссылки на документацию.

### Исправлено
- Небольшие правки формулировок и опечаток в документации.

## [0.9.2] - 2025-09-12

### Добавлено
*   **Новый модуль "Трекер привычек":**
    *   **"Интервью" для создания привычек:** Пользователь может описать желаемые привычки естественным языком в одном сообщении. LLM анализирует текст и автоматически формирует список привычек с названием, частотой (RRULE) и временем напоминания.
    *   **Подтверждение и управление:** Созданный список показывается пользователю для подтверждения. Реализован интерфейс для управления привычками, включая их удаление.
    *   **Автоматические напоминания:** Планировщик `APScheduler` создает персонализированные `cron` задачи для каждой привычки, учитывая ее правило повторения (ежедневно, по дням недели и т.д.) и часовой пояс пользователя.
    *   **Интерактивный трекинг:** В уведомлении о привычке пользователь может отметить ее как "✅ Выполнено" или "❌ Пропущено". Эта информация сохраняется в базу данных для статистики.
    *   **Еженедельный отчет о прогрессе:** Каждое воскресенье бот автоматически формирует и отправляет пользователю персональный отчет. В отчете содержится "прогресс-бар" из эмодзи за последние 7 дней для каждой привычки и общая процентная статистика.
*   **Новые достижения (ачивки):**
    *   Добавлены 7 новых достижений, связанных с использованием продвинутых функций бота и трекером привычек.
    *   Реализованы триггеры для "тихой" и мгновенной выдачи достижений при выполнении соответствующих действий (например, при изменении настроек или просмотре гайдов).
*   **Утилита для форматирования RRULE:** Создана функция `format_rrule_for_user` для преобразования технических строк iCalendar (например, `FREQ=WEEKLY;BYDAY=MO,FR`) в человекочитаемый формат ("По дням: пн, пт").

### Изменено
*   **База данных:**
    *   Добавлены новые таблицы `habits` (для хранения привычек) и `habit_trackings` (для статистики выполнения).
    *   В таблицу `users` добавлено поле `viewed_guides` (JSONB) для отслеживания просмотренных гайдов.
    *   В таблицу `notes` добавлено поле `snooze_count` (INTEGER) для отслеживания откладываний напоминаний.
*   **Главное меню:** Добавлена кнопка "💪 Мои привычки" для доступа к новому модулю.

### Исправлено
*   **Планировщик:** Устранена критическая ошибка `InvalidParameterValueError: time zone "Europe/Kiev" not recognized` в PostgreSQL, возникавшая из-за устаревшего названия часового пояса у некоторых пользователей.
*   **Геймификация:** Исправлена логика выдачи достижений за изменение настроек. Теперь ачивки "Повелитель времени" и "Мой адрес не дом и не улица" выдаются корректно при первом изменении, а не только при первой установке.
*   **Парсер RRULE:** Исправлена ошибка `AttributeError: 'int' object has no attribute 'weekday'`, возникавшая из-за разных форматов хранения дней недели в `dateutil.rrule`.

## [0.9.1] - 2025-09-11

### Изменено
- Улучшена структура документации проекта; `CHANGELOG.md` теперь содержит раздел Unreleased.

### Исправлено
- Уточнены требования и переменные окружения в `README.md`.

## [0.9.0] - 2025-07-11

### Добавлено
*   **Проактивный AI-ассистент (⭐ VIP-функция):**
    *   Бот научился анализировать расписание пользователя. Если новая задача по времени **конфликтует** с уже существующей, AI дополнительно оценивает их совместимость по сути.
    *   Предупреждение отправляется пользователю только в случае **реального конфликта** (например, "встреча в офисе" и "поездка к врачу"), а не для совместимых задач ("позвонить маме" и "проверить почту").
*   **Система геймификации:**
    *   Внедрена система **опыта (XP) и уровней**. Пользователи получают опыт за большинство полезных действий (создание и выполнение заметок, добавление дней рождения и т.д.).
    *   Добавлена система **достижений (ачивок)**, которые выдаются за достижение определенных рубежей ("Создать 10 заметок", "Поделиться заметкой" и т.п.).
    *   В профиле пользователя (`/profile`) теперь отображается его уровень, прогресс-бар опыта и добавлена кнопка **"🏆 Мои достижения"** для просмотра всех полученных и доступных ачивок.
*   **"Тихая" миграция для геймификации:** Для существующих пользователей был разработан механизм, который начисляет им опыт и достижения за прошлые действия без отправки массовых уведомлений.

### Изменено
*   **Производительность и отзывчивость (Архитектурное обновление):**
    *   **Фоновая обработка:** Все "тяжелые" операции (распознавание речи, AI-анализ) теперь выполняются в фоновом режиме. Бот отвечает на сообщения **мгновенно**, а результат обработки присылает позже, редактируя исходное сообщение.
    *   **Кэширование на Redis:** Профили пользователей и другие часто запрашиваемые данные теперь кэшируются в Redis, что значительно ускоряет работу интерфейса и снижает нагрузку на основную базу данных.
    *   **Персистентность FSM:** Хранилище состояний (FSM) перенесено из оперативной памяти в Redis. Теперь все многошаговые диалоги (например, добавление дня рождения) не прерываются при перезапуске бота.
*   **Логика AI-анализа (Декомпозиция):**
    *   Процесс анализа текста в LLM был разбит на несколько этапов (Chain of Thought). Теперь AI сначала определяет **намерение пользователя** (заметка, список покупок, напоминание), а затем вызывает специализированную модель для **извлечения деталей**. Это кардинально повысило точность и надежность анализа.
*   **База данных:**
    *   В таблицу `users` добавлены поля `xp` (BIGINT) и `level` (INTEGER) для системы геймификации.
    *   Добавлены новые таблицы `achievements` (справочник достижений) и `user_achievements` (связка пользователя с полученной ачивкой).
    *   Добавлены новые функции в репозитории для подсчета статистики, необходимой для выдачи достижений (`count_total_and_voice_notes`, `find_conflicting_notes` и др.).

### Исправлено
*   Устранена ошибка `AttributeError: module 'aiogram.types' has no attribute 'Bot'`, возникавшая при вызове фоновых задач.
*   Исправлена ошибка валидации `pydantic` при вызове `bot.edit_message_text` с позиционными аргументами. Теперь используются именованные аргументы для большей надежности.
*   Устранена потенциальная ошибка `ValueError` при определении намерения пользователя, когда LLM возвращала синоним. Enum `UserIntent` теперь более гибко обрабатывает ответы модели.



## [0.8.0] - 2025-07-07

### Добавлено
*   **Интерактивные списки покупок (⭐ VIP-функция):** 
    *   Реализован новый тип заметки "Покупки". Если в сообщении есть ключевые слова (например, "купить", "список продуктов"), LLM автоматически присваивает заметке эту категорию.
    *   Создан **единый (персистентный) список покупок** для пользователя. Все новые товары добавляются в этот один список, а не создают новую заметку.
    *   В режиме просмотра заметки-списка появляется интерактивная клавиатура, где можно отмечать купленные товары (✅/⬜️), не редактируя текст.
    *   После завершения покупок список можно заархивировать одной кнопкой.
    *   В профиле пользователя теперь динамически появляется кнопка "🛒 Список покупок" для быстрого доступа, если у него есть активный список.

*   **Суммаризация заметок (Заголовки):**
    *   LLM теперь генерирует не только полный исправленный текст, но и короткий заголовок-выжимку (`summary_text`) для каждой заметки.
    *   Этот заголовок используется в общем списке заметок для более удобного и быстрого просмотра.


*   **Интеграция с Яндекс.Алисой (⭐ VIP-функция):**
    *   Реализован веб-сервер на FastAPI для приема команд от навыка Алисы.
    *   Пользователи могут привязать свой Telegram-аккаунт к Алисе с помощью команды в настройках.
    *   Для безопасной привязки используется система одноразовых кодов, действительных 10 минут.
    *   Добавлена возможность создавать заметки голосом через Алису, которые автоматически сохраняются в боте.

*   **Система поддержки (тикет-система):**
    *   В главное меню добавлена кнопка "Сообщить о проблеме".
    *   Пользователи могут отправить сообщение (текст, фото) с описанием проблемы или предложением.
    *   Администратор получает структурированное уведомление и пересланное сообщение от пользователя.
    *   Администратор может ответить на пересланное сообщение, и бот доставит ответ напрямую пользователю. Для этого используется FSM для надежного отслеживания цепочки "репорт-ответ".

*   **Мгновенное создание заметок из сообщений:**
    *   Любое текстовое сообщение (не команда), а также пересланное сообщение, теперь автоматически обрабатывается как новая заметка.
    *   Голосовые сообщения также создают заметку автоматически, без необходимости подтверждения.
    *   Сразу после автоматического создания заметки под сообщением с результатом появляется кнопка "❌ Отменить", позволяющая быстро удалить ошибочно созданную заметку.

*   **Кнопка "Получить VIP бесплатно":**
    *   Для новых пользователей в главном меню появилась промо-кнопка для мгновенной самостоятельной активации VIP-статуса. Кнопка исчезает после получения статуса.

*   **Логирование действий пользователя:**
    *   Внедрена новая таблица `user_actions` для сбора базовой аналитики по использованию ключевых функций: регистрация, создание заметок, запросы VIP и т.д.

### Изменено

*   **Рефакторинг создания заметки:** Логика обработки текста/голоса, анализа через LLM и сохранения в БД вынесена в единый сервис `services/note_creator.py`. Это унифицировало создание заметок из разных источников (голос, текст, Алиса).
*   **Улучшен парсинг ответа LLM:** Функция `_parse_llm_json_response` теперь умеет очищать ответ от обрамляющих блоков кода (```json ... ```), что повышает стабильность анализа.
*   **Переработано главное меню:** Клавиатура теперь динамически формируется с помощью `InlineKeyboardBuilder`, что позволяет скрывать кнопку "Получить VIP" для тех, у кого он уже есть.
*   **Структура проекта:** Добавлены новые модули `handlers/support.py` для системы поддержки и `alice_webhook.py` для интеграции с Алисой.
* *   **Логика создания заметок:** Сервис `note_creator` был кардинально переработан для поддержки списков покупок. Теперь он проверяет категорию от LLM и, если это "Покупки", вызывает функцию `get_or_create_active_shopping_list_note` для обновления существующего списка, а не создания новой заметки.
*   **Системный промпт LLM:** Промпт для DeepSeek был значительно улучшен. Добавлены строгие инструкции по созданию `summary_text` и детальное описание формата для извлечения `items` (товаров) для списков покупок.
*   **Пользовательский интерфейс (UX):**
    *   Список заметок (`/my_notes`) теперь отображает иконку 🛒 для списков покупок и использует короткий `summary_text` для превью.
    *   В стартовом сообщении (`/start`) и при создании напоминания для пользователей с UTC-временем теперь показывается более настойчивое предупреждение о необходимости настроить часовой пояс.
    *   В профиле пользователя (`/profile`) теперь отображается иконка ⚠️ рядом с часовым поясом, если он установлен на UTC.
*   **Надежность:**
    *   Улучшена обработка ошибок в планировщике (`scheduler.py`): теперь бот корректно обрабатывает ситуации, когда пользователь его заблокировал (`TelegramForbiddenError`), и не падает.
    *   Улучшена генерация утренних дайджестов: добавлена обработка ошибок парсинга HTML, и в случае проблемы сводка будет отправлена без форматирования.

### Изменения в базе данных

*   В таблицу `notes` добавлена новая колонка `summary_text TEXT` для хранения коротких заголовков заметок. Для обновления существующей БД необходимо выполнить SQL-запрос:
    ```sql
    ALTER TABLE notes ADD COLUMN IF NOT EXISTS summary_text TEXT;
    ```

### Исправлено

*   В `inline_keyboards.py` исправлен импорт `aiogram.types`, который мог вызывать ошибки в некоторых конфигурациях.
*   Логика редактирования заметки (`notes.py`) улучшена для более корректного возврата к экрану просмотра после отмены редактирования.

## [0.7.0] - 2025-06-21

### Добавлено

*   **Повторяющиеся задачи (⭐ VIP-функция):**
    *   Реализована полноценная система повторяющихся напоминаний. Бот теперь понимает фразы "каждый день", "каждую неделю", "каждый месяц" и т.д.
    *   После срабатывания напоминание автоматически переносится на следующую дату согласно правилу.
    *   В интерфейсе просмотра заметки теперь отображается статус её повторения, и для VIP-пользователей доступна кнопка, чтобы сделать задачу разовой.
*   **Утренняя сводка / Дайджест (⭐ VIP-функция):**
    *   Создана система ежедневных дайджестов. Каждый день в 9:00 по локальному времени пользователя бот присылает сводку.
    *   Сводка генерируется с помощью LLM и включает в себя:
        *   Список задач, запланированных на сегодня.
        *   Список дней рождений, которые наступят в течение ближайших 7 дней.
    *   В настройках VIP-пользователи могут включать или отключать эту функцию.
*   **Улучшения для администратора:**
    *   Добавлена команда `/test_digest` для принудительного запуска генерации утренней сводки для себя в целях тестирования.

### Изменено

*   **Логика создания напоминаний:**
    *   Улучшено поведение при создании повторяющихся задач: если пользователь не указал дату начала (например, "каждый день пить воду"), бот автоматически установит старт на завтрашний день в стандартное время для напоминаний.
*   **Защита VIP-функций:**
    *   При попытке создать повторяющуюся задачу пользователь без VIP-статуса получит вежливое уведомление с предложением повысить статус.
    *   Если у пользователя истек VIP-статус, его повторяющиеся задачи автоматически становятся разовыми при следующем срабатывании.
*   **Забота о пользователях:**
    *   Утренняя сводка не будет отправляться пользователям, у которых в профиле установлен часовой пояс по умолчанию (UTC), чтобы избежать нежелательных уведомлений в ночное время.

### Исправлено

*   **Форматирование дайджеста:** Исправлен баг, из-за которого текст утренней сводки от LLM мог приходить в лишних кавычках или блоках кода, что ломало форматирование в Telegram. Теперь ответ модели проходит предварительную очистку.

### Безопасность

*   В `database_setup.py` добавлены недостающие функции для обновления данных заметки (`update_note_text`, `update_note_category` и др.), которые ранее были пропущены.


## [0.6.1-2] - 2025-06-20

### Добавлено

*   **Инструменты администрирования:**
    *   Для администратора добавлена команда `/broadcast` для массовой рассылки сообщений всем пользователям бота. Рассылка выполняется в фоновом режиме с контролем скорости для избежания блокировок со стороны Telegram.
*   **Улучшения UX:**
    *   Введен лимит на максимальную длительность голосового сообщения (30 секунд) для ускорения обработки и предотвращения злоупотреблений. Пользователь получает вежливое уведомление, если его сообщение слишком длинное.
*   **Мониторинг и логирование:**
    *   Добавлена интеграция с сервисом `Logtail` для централизованного сбора и анализа логов в реальном времени.
    *   Полностью переработана система логирования в `main.py` для более чистого вывода, ротации файлов и уменьшения "шума" от сторонних библиотек.

### Исправлено

*   **База данных:** Устранена критическая ошибка `AttributeError: module 'database_setup' has no attribute 'set_user_timezone'`, возникавшая при попытке сменить часовой пояс. Добавлены недостающие функции для работы с настройками пользователя (`set_user_timezone`, `set_user_default_reminder_time`, `set_user_pre_reminder_minutes`).


## [0.6.0] - 2025-06-16

### Добавлено

*   **Напоминания о днях рождения:**
    *   Реализован новый раздел для хранения важных дат (дней рождения, годовщин).
    *   Бот автоматически создает ежегодные напоминания для сохраненных дат.
    *   Для **VIP-пользователей** добавлена возможность массового импорта дат из текстового файла.
*   **Экспорт данных (⭐ VIP-функция):**
    *   VIP-пользователи теперь могут экспортировать свои заметки.
    *   Добавлен выбор формата экспорта (**JSON, CSV, TXT**) и набора данных для выгрузки (активные, архивные, все).
*   **Система донатов:**
    *   В информационный раздел добавлена возможность поддержать проект через донат (ЮMoney).
    *   Реализована страница с инструкцией и динамической подстановкой Telegram ID пользователя для удобства.
*   **Улучшенный онбординг:**
    *   В стартовое сообщение (`/start`) добавлен блок, информирующий новых пользователей о необходимости установить свой часовой пояс.
    *   Для пользователей с часовым поясом по умолчанию (UTC) в главном меню появляется дополнительная кнопка для быстрой настройки.
*   **Информационный раздел:**
    *   Создан раздел "ℹ️ Инфо & Помощь" с подробной инструкцией по использованию, описанием VIP-функций и ссылками на ресурсы проекта.

### Изменено

*   **Позиционирование VIP-функций:**
    *   Полностью переработаны тексты в настройках и на лендинг-странице. Теперь акцент делается на том, что базовые функции доступны всем, а VIP — это расширение возможностей, а не платный доступ.
*   **Логика определения времени:**
    *   Внутренняя логика бота (не LLM) теперь выполняет "санитарную проверку" даты. Если LLM вернул время, которое уже прошло сегодня (например, "8 утра" для вечернего сообщения), бот автоматически переносит напоминание на следующий день.
*   **Промпт для LLM:**
    *   Системный промпт для DeepSeek был доработан для более точного извлечения даты с учетом часового пояса пользователя, но основная логика переноса даты вынесена в код бота для надежности.
*   **Уведомления о смене статуса:**
    *   Сообщение о выдаче VIP теперь перечисляет все доступные премиум-функции.
    *   При отзыве VIP-статуса у пользователя, его персональные настройки напоминаний автоматически сбрасываются к значениям по умолчанию.
*   **Пользовательский интерфейс (UX):**
    *   Устранены "тупики" в диалоге. После выполнения действий (сохранение заметки, отметка о выполнении) бот теперь всегда предлагает дальнейшие шаги, возвращая пользователя в меню или к списку заметок.

### Исправлено

*   **Некорректное отображение времени:** Исправлен баг, из-за которого в сообщении с предложением сохранить заметку дата отображалась в UTC, а не в часовом поясе пользователя.
*   **Ошибка при динамическом добавлении кнопок:** Устранена ошибка `AttributeError: 'InlineKeyboardMarkup' object has no attribute 'builder'`, возникавшая при попытке добавить кнопку настройки часового пояса в `/start`.


## [0.5.0] - 2025-06-15

### Добавлено


*   **Система статусов (Free & VIP):**
    *   Внедрена логика разделения пользователей на Free и VIP.
    *   Для Free-пользователей ограничены продвинутые функции напоминаний.
*   **Интерактивные уведомления о напоминаниях:**
    *   В уведомления добавлены инлайн-кнопки: "✅ Выполнено", "Отложить на 1 час", "Отложить на 3 часа".
    *   Реализованы хендлеры для выполнения, архивации и переноса (`snooze`) напоминаний прямо из уведомления.
*   **Расширенные настройки пользователя:**
    *   Пользователи теперь могут настраивать **время напоминаний по умолчанию** (для задач, где указана только дата).
    *   Добавлена настройка **предварительных напоминаний** (за 30 мин, 1 час, 24 часа до срока).
    *   Все настройки вынесены в единый раздел "⚙️ Настройки" в профиле пользователя.
*   **Система заявок на VIP-статус:**
    *   Для пользователей без VIP-статуса при попытке доступа к платным функциям отображается информационное сообщение.
    *   Добавлена кнопка "🚀 Отправить заявку на VIP", которая уведомляет администратора о желании пользователя получить доступ.
*   **Информационный раздел:**
    *   В главное меню добавлена кнопка "ℹ️ Инфо & Помощь".
    *   Создан новый раздел с подробной инструкцией "Как пользоваться", описанием VIP-возможностей и ссылками на новостной канал и чат проекта.
*   **Инструменты для отладки:**
    *   Для администратора добавлена команда `/jobs` для просмотра всех активных задач в планировщике `APScheduler`.

### Изменено

*   **Логика планировщика напоминаний:**
    *   Планировщик теперь учитывает VIP-статус пользователя: "умные" и предварительные напоминания создаются только для VIP.
    *   Исправлен баг, из-за которого время напоминания, установленное по умолчанию, не сохранялось в базе данных и некорректно отображалось в интерфейсе.
    *   Улучшена система создания и удаления задач в `APScheduler` для корректной обработки нескольких напоминаний (основного и предварительного) на одну заметку.
*   **Пользовательский интерфейс (UX):**
    *   После выполнения действий (сохранение/отмена заметки, выполнение задачи) пользователь теперь не остается в "тупике", а получает сообщение с главным меню или перенаправляется в обновленный список заметок.
    *   Убраны системные термины (например, `create_reminder`) из ответа LLM, который видит пользователь.
    *   Обновлены тексты уведомлений о выдаче/снятии VIP-статуса с перечислением всех актуальных функций.
*   **Структура проекта:**
    *   Логика настройки часовых поясов перенесена в новый, единый роутер `handlers/settings.py`.
    *   Добавлен новый роутер `handlers/info.py` для информационного раздела.

### Исправлено

*   **Ошибка CallbackData:** Устранена ошибка `ValueError: Separator symbol ':' can not be used`, возникавшая из-за передачи времени в `callback_data`.
*   **Ошибка `message can't be edited`:** Исправлена ошибка, возникавшая при попытке редактировать сообщение пользователя после ручного ввода данных в FSM.
*   Исправлен баг, из-за которого предварительные напоминания не создавались из-за неполной передачи данных в планировщик.

### Безопасность

*   При отзыве VIP-статуса у пользователя теперь автоматически сбрасываются все персональные настройки, связанные с платными функциями.


## [0.4.0] - 2025-06-14

### Добавлено

*   **Система напоминаний:**
    *   Реализована базовая система напоминаний. Если при создании заметки LLM распознает дату и время (`due_date`), бот автоматически запланирует напоминание.
    *   Интегрирован планировщик `APScheduler` для обработки отложенных задач.
    *   При старте бота все предстоящие напоминания загружаются из базы данных.
    *   Напоминания корректно отменяются при удалении или архивации заметки и восстанавливаются при извлечении из архива.
    *   Для задач, где указана только дата (без времени), напоминание по умолчанию устанавливается на 9:00 UTC.
    *   В уведомление о напоминании добавлена инлайн-кнопка для быстрого перехода к просмотру заметки.
*   **VIP-статус и Админ-панель:**
    *   Введена система **VIP-статуса**, который снимает все лимиты (на количество заметок и распознаваний).
    *   Создана **панель администратора**, доступная по ID из `.env`.
    *   Добавлена команда `/users` для админа, отображающая список всех пользователей с пагинацией.
    *   Добавлена команда `/admin <ID>` (или ответом на сообщение), которая показывает детальную информацию о пользователе и инлайн-кнопку для выдачи/отзыва VIP-статуса.
    *   Администратор получает уведомление об успешной смене статуса, а пользователь — информационное сообщение о получении или снятии VIP.
*   **Управление заметками:**
    *   Добавлена возможность **прослушать оригинальное аудио** для заметки, если оно было сохранено.
    *   Реализована система **категорий**. Пользователь может изменить категорию заметки, выбрав из предопределенного списка (`Общее`, `Работа`, `Личное` и т.д.).
*   **Новые зависимости:**
    *   Добавлена библиотека `apscheduler` для работы с отложенными задачами.

### Изменено

*   **Улучшено извлечение дат LLM:**
    *   Системный промпт для LLM был доработан для более точного извлечения дат и времени. Теперь в LLM передается точное время в UTC для корректного расчета относительных временных промежутков ("через 5 минут").
*   **Интерфейс и UX:**
    *   В профиле пользователя теперь отображается его VIP-статус и лимиты (или их отсутствие).
    *   В окне просмотра заметки отображается ее категория и добавлены новые кнопки действий ("Прослушать аудио", "Изменить категорию").
*   **База данных:**
    *   В таблицу `users` добавлено поле `is_vip (BOOLEAN)`.
    *   В таблицу `notes` добавлено поле `category (TEXT)`.
    *   Добавлена функция `get_all_users_paginated` для получения списка пользователей для админ-панели.

### Исправлено

*   **Полный цикл управления заметками:**
    *   Исправлено удаление заметки через FSM.



## [0.3.0] - 2025-06-12

### Добавлено

*   **Полный цикл управления заметками:**
    *   Реализована возможность **редактировать текст** существующих заметок через FSM.
    *   Добавлена функция **архивации** заметок, позволяющая убирать их из основного списка.
    *   Добавлена функция **восстановления** заметок из архива.
    *   Создан отдельный раздел для просмотра **архивированных заметок** с пагинацией.
    *   Внедрено **двухэтапное удаление** заметки (через подтверждение) для предотвращения случайных удалений.
*   **Поддержка часовых поясов:**
    *   Пользователи теперь могут **устанавливать свой часовой пояс** в настройках профиля.
    *   Добавлено меню с выбором из распространенных часовых поясов и возможностью ручного ввода.
    *   Все даты и время в интерфейсе бота (профиль, просмотр заметок) теперь отображаются с учетом **локального времени пользователя**.
*   **Новые зависимости:**
    *   Добавлена библиотека `pytz` для работы с часовыми поясами.

### Изменено

*   **Улучшен интерфейс:**
    *   Полностью переработан экран **профиля пользователя**: улучшена структура, добавлена информация о лимитах и текущем часовом поясе.
    *   **Клавиатуры действий с заметкой** стали контекстно-зависимыми: отображают разные кнопки для активных и архивированных заметок.
    *   В главное меню добавлена кнопка **"🗄️ Архив"**.
*   **База данных:**
    *   В таблицу `users` добавлено поле `timezone` (TEXT) для хранения часового пояса пользователя.
*   **Структура проекта:**
    *   Добавлен новый модуль `services/tz_utils.py` для утилит по работе с часовыми поясами.
    *   Добавлен новый роутер `handlers/timezone_selector.py` для обработки логики выбора часового пояса.
    *   В FSM (`states.py`) добавлены новые состояния для редактирования заметок и настройки профиля.

### Исправлено

*   Устранена **ошибка циклического импорта** (`circular import`) путем отказа от импорта экземпляра `bot` в хендлеры и использования `message.bot` / `callback_query.bot`.



## [0.2.0]- 2025-05-26

### Добавлено

* Команда help с описанием функционала бота
* Добавлена пагинация для списка заметок
* Добавлена возможность удалить заметку
* Добавлена возможность просмотреть заметку подробно
* Добавлен лимит на количество распознаваний в день

### Изменено
* Изменена STT модель на Yandex SpeechKit
* Изменена структура файлов проекта
* Изменена структура базы данных
* Изменена логика распознавания пустых сообщений

### Устарело


### Удалено


### Исправлено
* Исправлен баг с распознаванием пустых сообщений

### Безопасность


## [0.1.0] - 2025-05-21 (MVP+)

Это первоначальный MVP+ (Минимально жизнеспособный продукт Плюс) релиз Telegram-бота "VoiceNote AI".

### Добавлено

*   **Основная обработка голосовых заметок:**
    *   Пользователи могут отправлять боту голосовые сообщения.
    *   Транскрибация речи в текст (STT) с использованием внешнего API Hugging Face Space.
    *   Улучшение текста и извлечение информации с помощью DeepSeek LLM API:
        *   Исправление ошибок транскрибации STT, грамматики и пунктуации.
        *   Извлечение описаний задач/событий, дат/времени, упомянутых людей, мест и предполагаемых намерений.
*   **Управление заметками:**
    *   Пользователи могут сохранять обработанные голосовые заметки в базу данных PostgreSQL.
        *   Данные заметки включают: оригинальный текст STT, исправленный текст LLM, полный JSON-ответ от LLM с анализом, `file_id` аудиофайла Telegram и извлеченную дату выполнения (`due_date`).
        *   Дата/время оригинального голосового сообщения (`message.date`) сохраняется как `note_taken_at`.
    *   Пользователи могут просматривать свои последние 5 заметок через inline-кнопку "📝 Мои заметки". Каждая заметка отображается отдельно с возможностью удаления.
    *   Пользователи могут удалять отдельные заметки.
    *   Реализован MVP-лимит в 5 активных заметок на пользователя.
*   **Профиль пользователя:**
    *   Пользователи могут просматривать базовый профиль через inline-кнопку "👤 Профиль".
    *   Профиль отображает Telegram ID, имя пользователя (username), имя, дату регистрации, заглушку для статуса подписки ("Бесплатная (MVP)") и текущее количество заметок относительно MVP-лимита.
*   **Регистрация пользователя:**
    *   Автоматическая регистрация/обновление пользователя в базе данных при первом взаимодействии (`/start` или первое сообщение).
*   **База данных:**
    *   Бэкенд на PostgreSQL с использованием драйвера `asyncpg`.
    *   Схема включает таблицы: `users`, `notes`, а также заготовки для `subscriptions`, `payments`, `user_subscriptions`.
    *   Автоматическое создание таблиц при запуске бота, если они не существуют.
    *   Тип подписки по умолчанию "Full Месячная" добавлен в таблицу `subscriptions` для будущего использования.
*   **Взаимодействие с ботом:**
    *   Inline-клавиатуры для основных действий ("📝 Мои заметки", "👤 Профиль") и действий с конкретными заметками ("🗑️ Удалить").
    *   Реализована FSM (Конечный автомат) для двухэтапного процесса сохранения заметки (подтверждение).
    *   Пользователь возвращается в главное меню после создания или удаления заметки.
*   **Конфигурация и развертывание:**
    *   Конфигурация через переменные окружения (токен Telegram, API-ключ DeepSeek, параметры подключения к БД).
    *   Использование `python-dotenv` для загрузки переменных окружения при локальной разработке.
    *   Предоставлен `Dockerfile` для сборки Docker-образа для развертывания.
    *   `requirements.txt` для зависимостей Python.
*   **Структура проекта и документация:**
    *   Модульная структура кода (`main.py`, `database_setup.py`, `llm_processor.py`, `utills.py`, `inline_keyboards.py`).
    *   Базовые строки документации (docstrings) в Python-файлах.
    *   Начальные версии `README.md` и этого `CHANGELOG.md`.

### Изменено

*   

### Исправлено

*   Начальные проблемы с `asyncpg` и типами данных JSON/JSONB решены путем обеспечения сериализации Python dict в строки JSON перед вставкой/обновлением в базе данных.